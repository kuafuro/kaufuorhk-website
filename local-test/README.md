...
- **最勁最準，慢唔緊要**：Whisper large-v3 用 **full precision**（CPU float32／GPU float16，唔用 int8 量化）＋ beam_size 8；SenseVoice 用公開版最勁（SenseVoiceSmall，fp32）。
- **Mac**：`faster-whisper`（CTranslate2）行 CPU float32——因為追準，幾分鐘片可能要行到差唔多片長咁耐，係正常。SenseVoice 行 MPS。趕時間先至考慮 `mlx-whisper`，追準就留返 float32。
