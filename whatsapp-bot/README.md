...
**同場加建第二個 template（取消堂通知）**:Holder 喺網站取消時段,已報名學員會自動收通知。

- Name: `class_cancelled` · Category: **Utility** · Language: **zh_HK**
- Body（`{{1}}`–`{{4}}` 同樣係 姓名/日期/時間/地點):

```
{{1}} 你好,唔好意思 — 原定 {{2}} {{3}} 喺{{4}}嗰堂已經取消 🙏
唔會扣你堂數。有問題直接搵教練啦。
— Kuafuor Motion Lab
```

（對應環境變數 `WHATSAPP_CANCEL_TEMPLATE_NAME`,預設 `class_cancelled`。）
