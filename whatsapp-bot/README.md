# Kuafuor WhatsApp Bot（對話式報堂 + 上堂提醒）

一個 WhatsApp bot,做兩件事:
...
> DB 側（`bot_book_slot` / `bot_cancel_booking` / `bot_my_bookings` / `due_reminders` 等 RPC、`whatsapp_reminders` 表）**已經套用落 Supabase**。
> 填好環境變數（尤其 `WHATSAPP_TOKEN` / `SUPABASE_SERVICE_ROLE_KEY`）+ 部署,就上到線。

---

## 對話式報堂點運作

- **官方 Cloud API 唔支援 group** — 所以係 **1 對 1 私訊**:學生 send 個 bot 號,唔係入 group。
- 學生要**先喺 kuafuorhk.com/login 帳戶頁填 WhatsApp 電話**（寫入 `profiles.phone`），bot 先認到佢。認唔到會叫佢去填。
- 對話：send 任何嘢 → 出「📅 報堂 / 📋 我嘅堂」兩個掣;撳「報堂」→ 未來 14 日開放時段清單 → 撳一堂即報;撳「我嘅堂」→ 撳一項即取消。
- **報名 = 排位**;實際扣堂/出席仍由教練喺分成計數機點名（同網站報名一致）。
- 程式:`src/booking.js`（純邏輯 + 依賴注入,`test/booking.test.js` 有 25 個單元測試）、`src/deps.js`（接 Supabase / WhatsApp）、`src/index.js` webhook `POST /webhook`。
